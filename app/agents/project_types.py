...
class ScriptBuilder(BaseProjectBuilder):
    ...
    def synthesize_starter_files(self) -> dict[str, str]:
        script_code = (
            ...
            '    return 0\n\n\n'
            'def main() -> int:\n'
            '    parser = argparse.ArgumentParser(description="' + self.goal.replace('"', '\\"') + '")\n'
...
def detect_project_type(goal: str, requirements: list[str] | None = None) -> BaseProjectBuilder:
    ...
    if any(k in combined for k in ["website", "landing page", "web page", "html", "portfolio", "css", "dark mode", "frontend", "dashboard"]):
        return WebsiteBuilder(goal, requirements)
    elif any(k in combined for k in ["fastapi", "rest api", "backend", "database", "sqlite", "service", "endpoint", "crud"]):
