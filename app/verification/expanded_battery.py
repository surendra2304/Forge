...
class SecurityVulnerabilityChecker:
    ...
    SECRET_PATTERNS = [
        (re.compile(r"""(?i)(?:api[_-]?key|secret|password|auth_token)\s*=\s*['"][a-zA-Z0-9_\-]{16,}['"]"""), "Potential hardcoded API key or secret"),
        (re.compile(r"""-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----"""), "Hardcoded private key block"),
    ...
    async def run_check(self, task_id: str, engine: ExecutionEngine) -> VerificationEvidence:
                    ...
                    for pattern, desc in self.SECRET_PATTERNS:
                        if pattern.search(content):
                            ...
                            if "test" not in rel_path.lower():
                                issues.append({"file": rel_path, "type": "hardcoded_secret", "description": desc})

                    # Check dangerous calls in Python
                    if f.suffix == ".py" and "test" not in rel_path.lower():
                        for call, desc in self.DANGEROUS_CALLS:
                            if call in content:
                                issues.append({"file": rel_path, "type": "unsafe_call", "description": desc})
                except Exception:
                    pass
        ...
        for d in dep_issues:
            issues.append({"type": "vulnerable_dependency", "package": d["package"], "description": d["reason"]})

        duration_ms = (time.perf_counter() - start_time) * 1000.0
        passed = len(issues) == 0

        stdout = f"Security scan completed across {len(inspected)} files. {len(issues)} issues detected."
        return VerificationEvidence(
            check_name=self.name,
            category=self.category,
